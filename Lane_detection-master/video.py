...
def grayscale(img):
    ...
    return cv2.cvtColor(img,
                        cv2.COLOR_RGB2GRAY)  # if you read an image with cv.imread(),return cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
...
def pipeline(img ,vertices ,low_thres ,hi_thres ,hline_show):
    ...
    h_lines = hough_lines(masked, rho=1, theta=np.pi / 180, threshold=30, min_line_len=10, max_line_gap=10)
...
def process_image(image):
    ...
    # imshape = [720,1280] #[367,639]
    imshape = image.shape
    vertices = np.array([[(imshape[1]*0.20,imshape[0]*0.6),(imshape[1]*0.4,imshape[0]*0.6),\
                         (imshape[1]*0.9,imshape[0]*0.80),(imshape[1]*0.10,imshape[0]*0.90),\
                         ]],dtype=np.int32)
    #白天 [[(imshape[1]*0.20,imshape[0]*0.3),(imshape[1]*0.80,imshape[0]*0.3),\
    #                          (imshape[1]*0.20,imshape[0]*0.70),(imshape[1]*0.80,imshape[0]*0.90),\
    #                          ]]
    low_thres,hi_thres = [20,80]
    return pipeline(image,vertices,low_thres,hi_thres,hline_show)

dir_video = 'test_videos/'
challenge_output = dir_video +'testOut_night.mp4'
